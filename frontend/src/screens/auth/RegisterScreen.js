/*
Intro: RegisterScreen es un pantalla del frontend de SubastUP.
Responsabilidad: organiza la logica visual y de estado definida en screens/auth/RegisterScreen.js.
...
import { Ionicons } from '@expo/vector-icons'; // Explica: importa dependencias desde '../../constants/colors'.
...
export default // Explica: declara la funcion RegisterScreen que concentra una parte del flujo.
function RegisterScreen({ navigation }) {// Estado para controlar la pestaña activa
  ...
  const step1Data = useRegisterStore((state) => state.step1Data); // Explica: define setStep1Data usando el resultado de useRegisterStore.
  ...
  const onSubmit = async (data) => {// Guardar datos en el store (incluye password)
    // Estado: actualiza un valor usado por la interfaz.
    setStep1Data(data); // Navegar al Paso 2
    navigation.navigate('RegisterStep2');}; // Render: devuelve el resultado que consume React o la funcion llamadora.
    ...
    <KeyboardAvoidingView
      ...
      <SafeAreaView style={styles.flex}>
          ...
          {/* Paso 2 - CLICKEABLE (inactivo) */}
            <TouchableOpacity style={styles.stepItem} onPress={() => {// Explica: ejecuta una accion como parte del flujo.
                handleSubmit((data) => {// Estado: actualiza un valor usado por la interfaz.
                    setStep1Data(data); // Navegacion: cambia de pantalla o ajusta opciones de navegacion.
        ...
        {/* Campo: Nombre */}
          ...
          <Controller control={control} name="nombre" rules={{ required: 'El nombre es obligatorio' }} render={({ field: { onChange, onBlur, value } }) => // UI: permite ingresar o editar texto.
            <TextInput style={[styles.input, errors.nombre && styles.inputError]} onBlur={onBlur}
        ...
        {errors.nombre && // UI: muestra texto visible en la pantalla.
          <Text style={styles.fieldError}>{errors.nombre.message}</Text>}

        {/* Campo: Apellido */}
          <Text style={styles.label}>Apellido</Text>
          <Controller control={control} name="apellido" rules={{ required: 'El apellido es obligatorio' }}
        ...
        {errors.apellido && // UI: muestra texto visible en la pantalla.
          <Text style={styles.fieldError}>{errors.apellido.message}</Text>}

        {/* Campo: DNI */}
          <Text style={styles.label}>DNI</Text>
          <Controller control={control} name="dni" rules={{ required: 'El DNI es obligatorio' }}
            render={({ field: { onChange, onBlur, value } }) => // UI: permite ingresar o editar texto.
            <TextInput
        ...
        {errors.dni && // UI: muestra texto visible en la pantalla.
          <Text style={styles.fieldError}>{errors.dni.message}</Text>}

        {/* Campo: Teléfono */}
          <Text style={styles.label}>Teléfono</Text>
          <Controller control={control} name="telefono" rules={{ required: 'El teléfono es obligatorio' }}
            render={({ field: { onChange, onBlur, value } }) => // UI: permite ingresar o editar texto.
            <TextInput
        ...
        {errors.telefono && // UI: muestra texto visible en la pantalla.
          <Text style={styles.fieldError}>{errors.telefono.message}</Text>}
        ...
        {/* Campo: Confirmar Contraseña */}
          ...
          <View style={styles.passwordWrapper}>
            <Controller control={control} name="confirmPassword" rules={{ required: 'Confirmá tu contraseña',
                validate: (val) =>
        ...
        {errors.confirmPassword && // UI: muestra texto visible en la pantalla.
