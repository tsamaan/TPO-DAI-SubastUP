...
// ── Configuración ──────────────────────────────────────────────
const BASE_URL = 'https://tpo-dai-subastup.onrender.com'
...
function Mensajes({ token, toast }) {
  ...
  const fetchConvs = useCallback(async () => {
    setLoading(true)
    const data = await api('/api/chat', 'GET', null, token)
  ...
  const fetchMsgs = async (convId) => {
    setSelected(convId)
    const data = await api(`/api/chat/${convId}`, 'GET', null, token)
    setMsgs(data.mensajes || [])
  }

  const enviar = async () => {
    if (!texto.trim() || !selected) return
    const data = await api(`/api/chat/${selected}/mensaje`, 'POST', { texto }, token)
...
function Bienes({ token, toast }) {
  ...
  const fetchBienes = useCallback(async () => {
    setLoading(true)
    const data = await api('/api/productos/revision/pendientes', 'GET', null, token)
  ...
  const rechazar = async (id) => {
    ...
    const data = await api(`/api/productos/${id}/rechazar`, 'PUT', { motivo: motivos[id], cargo: 0 }, token)
...
function MetodosPago({ token, toast }) {
  ...
  const fetchMetodos = useCallback(async () => {
    setLoading(true)
    const data = await api('/api/pagos/pendientes-verificacion', 'GET', null, token)
  ...
  const verificar = async (id) => {
    const data = await api(`/api/pagos/${id}/verificar`, 'PUT', null, token)
